...
function emailAppointmentReminder24HBefore(emailList, idList){
          ...
          for(var i=0;i<emailList.length;i++){
            ...
            Event.updateMany({_id:{$in:idList}}, {reminded:true}, function(errorUpdate, resultObjUpdate){
              if(errorUpdate){
...
function smsAppointmentReminder24hBefore(smsList, idList){
  ...
  twilioService.notifications.create({
    toBinding:bindings,
    body:"Dailyworks here! Just a friendly reminder of your upcoming appointment. Vist www.dailyworks.ca/appointmentDetails for the details."
  }).then(function(notification){
    if(notification.sid){
      Event.updateMany({_id:{$in:idList}}, {reminded:true}, {upsert:false}, function(errorUpdate, resultObjUpdate){
        if(errorUpdate){
          console.log(errorUpdate);
        }
      });
...
module.exports.appointmentReminderFinder = function(){
  ...
  Event.find({startDate:{"$gte":reminderStartDate}, endDate:{"$lte":reminderEndDate}, reminded:false}, {_id:1, clientId:1, clientName:1, reminderPhone:1, reminderEmail:1, storeName:1}, function(errorFind, resultAppointmentList){
    if(errorFind){
...
module.exports.deleteOlderUnconfirmedAppointmentsAndClients = function(){
  ...
  UnconfirmedGuestClient.deleteMany({dateCreated:{"$lte":date12HAgo}}, function(errorDelete){
    if(!errorDelete){
      console.log("Successfully deleted all old unconfirmed guest clients.");
    }
  });
