...
app.get('/home', sessionMiddlewares.loggedInCheck, function (req, res) {
  if (res.locals.userType == 'store') {
    ...
    Event.find(
      { clientId: mongoose.Types.ObjectId(res.locals.userId) },
      { _id: 1, storeName: 1, employeeName: 1, startDate: 1, endDate: 1, fullDay: 1, services: 1 },
      ...
      function (err, eventsObj) {
        if (err) {
          ...
          Client.findOne({ _id: mongoose.Types.ObjectId(res.locals.userId) }, { _id: 0 }, function (
            error,
            clientObj
          ) {
            if (error) {
