...
app.post('/postLocalClientLogin', sessionMiddlewares.denyIfLoggedIn, function (req, res) {
  ...
  Client.findOne({ email: email, provider: null }, { password: 1, _id: 1, email: 1 }, function (
    errorFind,
    currentUser
  ) {
    if (!errorFind) {
      if (currentUser) {
        ...
        bcrypt.compare(password, currentUser.password, function (err, respBcrypt) {
          ...
          if (err) {
            ...
            //pass match
            if (respBcrypt == true) {
              cookieFunctions.createSessionCookie(currentUser._id, 'client', res, function () {
                res.send('ok');
              });
            } else {
...
app.post('/postLocalStoreLogin', sessionMiddlewares.denyIfLoggedIn, function (req, res) {
  var email = req.body.email;
  var password = req.body.password;
  Store.findOne({ email: email }, { password: 1, _id: 1, email: 1 }, function (
    errorFind,
    currentUser
  ) {
    if (!errorFind) {
      if (currentUser) {
        ...
        bcrypt.compare(password, currentUser.password, function (err, resBcrypt) {
          ...
          if (err) {
            ...
            //pass match
            if (resBcrypt == true) {
              cookieFunctions.createSessionCookie(currentUser._id, 'store', res, function () {
                res.send('ok');
              });
            } else {
              res.send('passwordNoMatch');
...
app.post('/postLocalStoreSignup', sessionMiddlewares.denyIfLoggedIn, function (req, res) {
  ...
  Store.findOne({ email: email }, { email: 1, _id: 1, password: 1 }, function (
    errorFind,
    currentUser
  ) {
    if (!errorFind) {
      if (currentUser) {
        ...
        bcrypt.hash(password, saltRoundsBcrypt, function (err, hash) {
          if (err) {
            ...
            var tasksArray = [0, 1];
            Store.create(
              { storeName: storeName, email: email, password: hash, registrationTasks: tasksArray },
              function (err, storeObj) {
                if (err) {
                  console.log(err);
                  res.send('err');
                } else {
...
app.post('/postLocalClientSignup', sessionMiddlewares.denyIfLoggedIn, function (req, res) {
  ...
  Client.findOne({ email: email, provider: null }, { _id: 0, email: 1 }, function (
    errorFind,
    currentUser
  ) {
    if (!errorFind) {
      if (currentUser) {
        ...
        bcrypt.hash(password, saltRoundsBcrypt, function (err, hash) {
          ...
          if (err) {
            ...
            User.create({ email: email, password: hash, clientName: clientName }, function (
              err,
              clientObj
            ) {
              if (err) {
...
app.get('/google/redirect', passport.authenticate('google', { session: false }), function (
  req,
  res
) {
  cookieFunctions.createSessionCookie(req.user.id, 'client', res, function () {
    res.redirect('/home');
  });
});
