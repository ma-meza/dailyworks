...
if (cluster.isMaster) {
  ...
  cron.schedule('0 */1 * * *', function () {
    cluster.workers[Object.keys(cluster.workers)[0]].send({
      msgType: 0,
      msgContent: 'do cron job unconfirmed apptmnts/guest clients',
    });
    cluster.workers[Object.keys(cluster.workers)[1]].send({
      msgType: 2,
      msgContent: 'do cron job apptmnts reminders',
    });
  });
  ...
  app.get('/1', function (req, res) {
    res.render('landingPageStorePreLaunch', {
  ...
  app.get('/getMoreEventsLater', sessionMiddlewares.loggedInCheck, function (req, res) {
    ...
    if (res.locals.userType == 'store') {
      Event.find(
        { storeId: mongoose.Types.ObjectId(res.locals.userId), startDate: { $gte: new Date() } },
        { _id: 1, clientName: 1, startDate: 1, endDate: 1, fullDay: 1, services: 1 },
        { sort: { startDate: 1 }, limit: nbEventsQueryLimit, skip: nbEventsAlreadyFetched },
  ...
  app.get('/getAllStoreEvents', sessionMiddlewares.loggedInCheck, function (req, res) {
    if (res.locals.userType == 'store') {
      ...
      Event.find(
        { storeId: mongoose.Types.ObjectId(res.locals.userId), startDate: { $gte: dateInLocal } },
        { _id: 1, clientName: 1, startDate: 1, endDate: 1, fullDay: 1, services: 1 },
        { sort: { startDate: 1 } },
        function (err, resultObj) {
          if (err) {
  ...
  app.get('/getAllStoreLocalGuestClients', sessionMiddlewares.loggedInCheck, function (req, res) {
    LocalGuestClient.find({ storeId: mongoose.Types.ObjectId(res.locals.userId) }, {}, function (
      error,
      clientsObj
    ) {
      if (error) {
  ...
  app.use(function (req, res, next) {
    return res
      .status(404)
      .render('404page.ejs', { languageCookie: cookieFunctions.getLanguageCookieValue(req, res) });
  });
