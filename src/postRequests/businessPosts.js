...
app.post('/removeNotificationAlertFromStore', sessionMiddlewares.loggedInCheck, function (
  req,
  res
) {
  Store.updateOne(
    ...
    function (errorUpdate, responseObj) {
      if (errorUpdate) {
        console.log('error');
...
app.post('/addStoreAbsence', sessionMiddlewares.loggedInCheck, function (req, res) {
  ...
  if (absenceObj) {
    Store.updateOne(
      { _id: mongoose.Types.ObjectId(res.locals.userId) },
      { $push: { storeAbsences: absenceObj } },
      function (errorUpdate, responseObjUpdated) {
        if (errorUpdate) {
          console.log(errorUpdate);
...
app.post('/deleteStoreAbsence', sessionMiddlewares.loggedInCheck, function (req, res) {
  ...
  Store.updateOne(
    { _id: mongoose.Types.ObjectId(res.locals.userId) },
    { $pull: { storeAbsences: { _id: mongoose.Types.ObjectId(absenceId) } } },
    function (errorUpdate, responseObjUpdated) {
      if (errorUpdate) {
        console.log(errorUpdate);
        res.send('error');
...
app.post('/deleteStoreService', sessionMiddlewares.loggedInCheck, function (req, res) {
  ...
  Store.updateOne(
    { _id: mongoose.Types.ObjectId(res.locals.userId) },
    { $pull: { services: { _id: mongoose.Types.ObjectId(serviceId) } } },
    function (errorUpdate, responseObjUpdated) {
      if (errorUpdate) {
        console.log(errorUpdate);
        res.send('error');
...
app.post('/updateStoreGeneralSettings', sessionMiddlewares.loggedInCheck, function (req, res) {
  ...
  if (oldPass != '' && newPass != '') {
    ...
    Store.updateOne({ _id: mongoose.Types.ObjectId(res.locals.userId) }, updateString, function (
      errorUpdate,
      responseObjUpdated
    ) {
      if (errorUpdate) {
...
app.post('/postNewAppointmentLocal', sessionMiddlewares.loggedInCheck, function (req, res) {
  ...
  if (req.body.clientInfos.newClient == true) {
    Event.create(
      {
        storeId: mongoose.Types.ObjectId(res.locals.userId),
        startDate: req.body.appointmentInfos.startDate,
        endDate: req.body.appointmentInfos.endDate,
        clientId: mongoose.Types.ObjectId(req.body.clientInfos.clientId),
        employeeId: employeeId,
        employeeName: employeeName,
        services: servicesObj,
        addNote: req.body.appointmentInfos.addNote,
      },
      function (errorEvent, eventObj) {
        if (errorEvent) {
    ...
    LocalGuestClient.create(
      {
        fullName: req.body.clientInfos.name,
        email: req.body.clientInfos.email,
        phoneNumber: req.body.clientInfos.phoneNumber,
        customerSince: new Date(),
        storeId: mongoose.Types.ObjectId(res.locals.userId),
      },
      function (errorClient, clientObj) {
        if (errorClient) {
